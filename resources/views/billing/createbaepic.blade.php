              ...
              <table id="ttable_id" class="table table-responsive">
                ...
                <tbody>
                  ...
                  <tr>
                    <td nowrap>{{ $no++ }}</td>
                    @php
                      $getCount = DB::connection('oci8')->table("CDM_BA_EPIC")->select(DB::raw('count(id_req) as jum'))->where('id_req',$datas->id_req)->get();
                      $count = $getCount[0]->jum;
                    @endphp
                    <td nowrap>
                      <button class="btn btn-warning" id="create_billing" value="{{ $datas->id }}" <?php if($count > '0'){ ?> disabled <?php } ?> >
...
<div class="modal fade" id="example" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form id="create-epic" action="{{ route('create-epic') }}" method="POST" class="request">
        {{ csrf_field() }}
        <div class="modal-body">
          ...
          <?php
            for($i = 0; $i < $datas->jumlah_ba; $i++ ) {
          ?>

            <div class="form-group">
              <label for="account_num">ACCOUNT NUM <?php echo ($i+1)?></label>
              <input id="account_num" type="text" name="account_num[]" class="form-control" >
            </div>
            <div class="form-group">
              <label for="site">SITE <?php echo ($i+1)?></label>
